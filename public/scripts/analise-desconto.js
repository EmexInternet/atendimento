                ...
                if(opcoesDesconto == 'semacesso'){
                    ...
                    if(tipo_prazo == 'corrido'){
                        ...
                        //TIPO UTEIS (MESMAS VARIAVEWIS DO CORRIDO POREM COM NOMES DIFERENTES)
                        {
                            ...
                            if(moment(dataInicioAtraso).add(dias_prazo_novo, 'days').weekday() == 6){

                                diasNaoUteis.push([data_inicio_atraso, moment(dataInicioAtraso).add(dias_prazo_novo, 'days').weekday()])
                                diasNaoUteis.push([moment(dataInicioAtraso).add(dias_prazo_novo+1, 'days').format('L'), moment(dataInicioAtraso).add(dias_prazo_novo+1, 'days').weekday()])

                                data_inicio_atraso = moment(data_inicio_atraso, "DD/MM/YYYY").add(2, 'days').format('L')
                                console.log('O ultimo dia do prazo caiu no sabado, sera adicionado mais dois dias de prazo')

                                
                            }else if(moment(dataInicioAtraso).add(dias_prazo_novo, 'days').weekday() == 0){
                                diasNaoUteis.push([data_inicio_atraso, moment(dataInicioAtraso).add(dias_prazo_novo, 'days').weekday()])


                                data_inicio_atraso = moment(data_inicio_atraso, "DD/MM/YYYY").add(1, 'days').format('L')
                                console.log('O ultimo dia do prazo caiu no domingo, sera adicionado mais um dia de prazo')
