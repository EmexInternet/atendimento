class Multa{
    ...
    gerarMulta(){
        ...
        const data_atual = moment(new Date(), 'YYYY-MM-DD').format('L')



        if(valor !== ''  && document.getElementById('data').value !== ''){

                if(moment(data_atual, "DD/MM/YYYY").isAfter(moment(data_venda, "DD/MM/YYYY"))){
                    const dia_atual = data_atual.split('/')[0];
                    const mes_atual = data_atual.split('/')[1];
                    const ano_atual = data_atual.split('/')[2];
                    ...
                    if(moment(data_atual, "DD/MM/YYYY").isAfter(moment(dataFimTeste, "DD/MM/YYYY"))){

                        console.log(dataFimTeste)

                        let qtd_meses_pagos = Math.round(moment(data_atual, "DD/MM/YYYY").diff(moment(data_venda, "DD/MM/YYYY"), "months"))
                        ...
                        if(meses_restantes <= 0){
                            ...
                            document.getElementById('mensagem').value = `CALCULO DA MULTA RECISÓRIA\n\n`+
                                                                        `${plural_singular} PARA O FIM DO CONTRATO FIDELIDADE\n\n`+
                                                                        `VALOR DO DA MULTA: R$ ${calculo_multa.toFixed(2)}`
                        }
