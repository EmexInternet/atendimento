class Vencimento{
  ...
  alterarModo(bool){
    ...
    if(bool){
      ...
      if(trocar){
        ...
        //Solução para arrumar o problema do "onClick" do React que nao alterava a função
        let divPai = document.getElementsByClassName('tab-content')[0]
        if (divPai.childNodes) {
          divPai.removeChild(document.getElementById('botao-gerar'));
        }

        let button = document.createElement('button')
        button.innerHTML = 'Gerar'
        button.id = 'botao-gerar'
        button.type = 'button'
        button.setAttribute("class", "subscribe btn btn-primary btn-block rounded-pill shadow-sm");
        button.style = 'width: 100px; height: 40px'
        button.setAttribute('onclick', "vencimento.menorMaior()")

        var copiar = document.querySelector("#copiar");

        copiar.parentElement.insertBefore(button, copiar);
      ...
      if(trocar){
        ...
        button.setAttribute("class", "subscribe btn btn-primary btn-block rounded-pill shadow-sm");
        button.style = 'width: 100px; height: 40px'
        button.setAttribute('onclick', "vencimento.maiorMenor()")
  ...
  menorMaior(){
    ...
    if(parseInt(dia_antigo) >= parseInt(dia_novo)){
      ...
      if(moment(data_contato).isBefore('2024-01-01')) {
        alert('Período de registro de contato indisponível! Selecione um ano igual ou superior ao ano atual');
        return; // Interrompe a execução adicional da função se a condição for verdadeira
      }    
      ...
      if(parseInt(dia_contato) < dia_antigo){ //antes de pagar o boleto do mesmo mes
        ...
        let ano_mes_inicio_contrato_antigo = `${ano_contato}/${mes_contato}`
  
        let dias_do_mes = moment(ano_mes_inicio_contrato_antigo, "DD/MM/YYYY").daysInMonth()
        ...
        console.log('aqui ' + dias_pre_pago)
        ...
        if(diferenca_dias <= prazo_faturamento){ // mesmo mês 17 27 09/11/22
          ...
          console.log('boleto já faturado, vai cair só mês que vem')//  12/27 01/02/2023 77.70
        ...
        console.log(data_contato + 'data aquiii')
        ...
        let dias_do_mes = moment(data_proxima_fatura, "DD/MM/YYYY").daysInMonth() //volta
        ...
        console.log(dias_pre_pago + 'olha aqui')
  
        let data_fim_contrato_prox_fatura = moment(data_proxima_fatura, "DD/MM/YYYY").add(dias_pre_pago, 'days')
  
        if(diferenca_contato_prox_vencimento <= prazo_faturamento){ // 02 17 31/10
  ...
  alteracaoVencimentoFaturadoMenorMaior(data_fim_contrato_antigo, dia_novo, valor, data_inicio_contrato_antigo, diferenca_dias, dia_antigo){ // 17 27 09/11    //e também cai o 03 27 31/10
    ...
    let comeco_data_contagem = moment(data_fim_contrato_antigo, "DD/MM/YYYY").add(0, 'days')// alteração danilo
    let comeco_data_contagem_string = moment(data_fim_contrato_antigo, "DD/MM/YYYY").add(0, 'days').format('L')
    ...
    let diferenca_datas_dias = moment.duration(diferenca_datas).asDays(); //leo
    ...
    if(diferenca_dias <= 14){ //14 dias ou menos
      ...
      data_inicio_contrato_antigo = moment(data_inicio_contrato_antigo, "DD/MM/YYYY").format('L')
      

      var mensagem = `QUEM ENTROU EM CONTATO (NOME + PARENTESCO):\n`+
      `CONTATO VIA  (  ) WHATSAPP (  )TELEFONE / Nº:\n`+
      `PROTOCOLO INFORMADO? (  )SIM (  )NÃO\n\n`+
  ...
  alteracaoVencimentoNaoFaturadoMenorMaior(data_inicio_contrato_antigo, dia_novo, valor, diferenca_dias, dia_antigo){ //17 27 01/11/2022 // e tambem 17 27 31/10
    let data_inicio_contagem = moment(data_inicio_contrato_antigo, "DD/MM/YYYY").format('L')
  ...
  maiorMenor(){ // 22 7 09/11/2022
    ...
    if(parseInt(dia_antigo) <= parseInt(dia_novo)){
      ...
      if(parseInt(dia_contato) < dia_antigo){ //antes faturado  //antes de pagar o boleto
        ...
        let dias_do_mes = moment(data_proxima_fatura, "DD/MM/YYYY").daysInMonth()
        ...
        if(diferenca_contato_prox_vencimento <= prazo_faturamento){ //7 2 31/10
          ...
          this.altVencimentoFaturadoMaiorMenor(data_proporcional, data_nova_proxima_fatura, diferenca_data_nova_data_antiga, valor_desconto, true, dia_antigo, dia_novo, data_proxima_fatura)
  
  
  
        }else{ //22 7 31/10 ou 27 17 28/11
  ...
  altVencimentoFaturadoMaiorMenor(data_proporci, data_nova_prox_fatura, diferenca_contato_vencimento_antigo, valor, bool, dia_antigo, dia_novo, data_proxima_fatura){
    ...
    if(!bool){ // não faturado
      ...
      var mensagem = 
      `QUEM ENTROU EM CONTATO (NOME + PARENTESCO):\n`+
      `CONTATO VIA  (  ) WHATSAPP (  )TELEFONE / Nº:\n`+
      `PROTOCOLO INFORMADO? (  )SIM (  )NÃO\n\n`+
      `TROCA DE VENCIMENTO DO DIA ${dia_antigo} PARA O DIA ${dia_novo}\n\n` + 
      ...
      data_proxima_fatura = moment(data_proxima_fatura, "DD/MM/YYYY").format('L')
      ...
      `CLIENTE SOLICITOU A TROCA 14 DIAS OU MENOS ANTES DO VENCIMENTO, APÓS O FATURAMENTO OCORRER\n\n` + 
      `O BOLETO DO DIA ${data_proxima_fatura} NO VALOR DE R$ ${valor} PRECISA SER PAGO NORMALMENTE\n\n` + 
